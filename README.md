...
**Full speech-to-speech voice.** The OpenAI Realtime API is already integrated for streaming STT/live captions, while Claude remains the sole reasoning and decision-making model. A future production extension could use full-duplex, interruption-capable speech-to-speech interaction while preserving the same deterministic refund tools and policy controls.
